...
function M.setup(opts)
	...
	vim.api.nvim_create_user_command("DirDiffTest", function()
		print("DirDiffTest from dirdiff(NotDirDiff from vim)!. Configuration value : " .. (opts.hide_same or "true"))
	end, {})
end
...
function H.scan_dir(path_a, path_b, depth)
	...
	for _, name in ipairs(sorted_keys) do
		...
		if node.type == "directory" then
			children = H.scan_dir(
				(type_a == "directory") and full_a or nil,
				(type_b == "directory") and full_b or nil,
				depth + 1
			)
